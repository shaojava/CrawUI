...
namespace UiLib
{
	...
	void CDialogBuilder::ParseMultiLanguage(CDuiString xml,CPaintManagerUI* pManager)
	{
		if(!pManager) return;
		if(!m_xml.LoadFromFile(xml)) return;
		CMarkupNode root = m_xml.GetRoot();
		if( !root.IsValid() ) return;
		LPCTSTR pstrClass = NULL;
		CDuiString pstrValue;
		CDuiString pstrDate;
		for( CMarkupNode node = root.GetChild() ; node.IsValid(); node = node.GetSibling() ) 
		{
			pstrClass = node.GetName();
			if( _tcscmp(pstrClass, _T("string")) == 0 ) 
			{
				pstrValue = node.GetAttributeValue(0);
				pstrDate = node.GetValue();
				pManager->AddStringRess(pstrValue,pstrDate);
			}
		}
	}
	...
	CControlUI* CDialogBuilder::Create(IDialogBuilderCallback* pCallback, CPaintManagerUI* pManager, CControlUI* pParent)
	{
				...
				if( pManager->GetPaintWindow() ) {
					...
					for( int i = 0; i < nAttributes; i++ ) {
						...
						if( _tcscmp(pstrName, _T("size")) == 0 ) {
							...
							pManager->SetUnLayerdWndShadow(_tcscmp(pstrValue, _T("true")) == 0);
						}
						else if( _tcscmp(pstrName,_T("glass")) == 0){
							pManager->SetWndGlassEffect(_tcscmp(pstrValue, _T("true")) == 0);
						}
		...
		if(pManager&&pManager->GetCurStylesName().IsEmpty() && pManager->GetStylesCount() > 0)
			pManager->SetCurStyles(0,false);
