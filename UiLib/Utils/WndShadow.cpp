...
#pragma warning(disable: 4311)	// temporrarily disable the type_cast warning in Win32
	SetWindowLongPtr(hParentWnd, GWLP_WNDPROC, (INT_PTR)ParentProc);
...
#pragma warning(default: 4312)
	...
	case WM_SIZE:
		if(pThis->m_Status & SS_ENABLED && !(pThis->m_Status & SS_DISABLEDBYAERO))
		{
			...
			{
				LONG lParentStyle = GetWindowLongPtr(hwnd, GWL_STYLE);
	...
	case WM_NCDESTROY:
		{
			...
			SetWindowLongPtr(hwnd, GWLP_WNDPROC,pThis->m_OriParentProc);
		}
		break;
...
void CWndShadow::Update(HWND hParent)
{
	...
	if(hbitmap == NULL || pvBits == NULL)return ;
...
void CWndShadow::Show(HWND hParentWnd,bool bForce)
{
	if(m_hWnd == NULL)
	{
		return ;
	...
	if((m_Status & SS_ENABLED) && !(m_Status & SS_DISABLEDBYAERO))	// Enabled
	{
		...
		if(WS_VISIBLE & lParentStyle || bForce)	// Parent visible
		{
