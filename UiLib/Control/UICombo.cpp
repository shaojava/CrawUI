...
void CComboWnd::FindBegin(int nLastPos,int nScrollRange)
{
...
CComboUI::CComboUI() : m_pDropWnd(NULL), m_iCurSel(-1), m_uButtonState(0)
{
	m_iCurSel = -1;
	m_iOldSel = -1;
	m_bNeedTab = true;
...
void CComboUI::SetLastScrollPos(int nPos,int nRange)
{
	m_nLastPos = nPos;
...
bool CComboUI::SelectItem(CDuiString szItemText, bool bTakeFocus)
{
	for(int iIndex = 0 ; iIndex < m_items.GetSize(); iIndex++)
	{
		CListLabelElementUI *pListItem =  static_cast<CListLabelElementUI*>(m_items[iIndex]);
		if(pListItem->GetText() == szItemText)
		{
...
bool CComboUI::Add(CDuiString strtext)
{
	m_VectorFont.push_back(strtext);
	return true;
}

void CComboUI::Sort(bool bSort)
{
	if(bSort)
...
void CComboUI::OnLoadStringList()
{
...
void CComboUI::RemoveAll(bool bUpdate)
{
    m_iCurSel = -1;
...
bool CComboUI::Activate()
{
    ...
    if( m_pDropWnd ) return true;
    m_pDropWnd = new CComboWnd();
    ASSERT(m_pDropWnd);
...
{
    CContainerUI::SetEnabled(bEnable,true);
...
void CComboUI::SetArrowWidth(int nArrowWidth)
{
...
{
	if (_tcscmp(pstrName, _T("arrownormal")) == 0)
...
void CComboUI::DoPaint(const RECT& rcPaint)
{
...
void CComboUI::PaintStatusImage()
{
...
void CComboUI::PaintText()
{
    ...
    if( m_iCurSel >= 0 ) {
        CControlUI* pControl = static_cast<CControlUI*>(m_items[m_iCurSel]);
		CPaintManagerUI *OldpManager = pControl->GetManager();
		if(OldpManager != m_pManager)
			pControl->SetManager(m_pManager,pControl->GetParent(),false);
        IListItemUI* pElement = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
        if( pElement != NULL ) {
            pElement->DrawItemText(rcText);
        }
        else {
            RECT rcOldPos = pControl->GetPos();
            pControl->SetPos(rcText);
            pControl->DoPaint(rcText);
            pControl->SetPos(rcOldPos);
        }
		if(OldpManager != m_pManager)
			pControl->SetManager(OldpManager,pControl->GetParent(),false);
    }
}

void CComboUI::SetWheelAble( bool bAble)
{
	m_bWheelReaction = bAble;
}

bool CComboUI::GetHweelAble()
{
	return m_bWheelReaction;
}

void CComboUI::PaintArrow()
{
