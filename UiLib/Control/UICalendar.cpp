...
#define UICALENDAR_DLG_H
	...
	public:
		...
		void SetCalendarName(LPTSTR lpStrName);
		void SetTrancateDate(LPCTSTR szBegin = NULL,LPCTSTR szEnd = NULL);
	private:
	...
	void CCalendarDlg::Notify( TNotifyUI& msg )
	{
		if(msg.sType == _T("OnSelcetDay") || msg.sType == _T("OnToday"))
		{
			...
			if(pCControl){
				...
				pCControl->GetManager()->SendNotify(pCalendar,_T("OnSelcetDay"));
			}
			Close(IDOK);
		}
	}
	...
	void CCalendarDlg::SetCalendarName( LPTSTR lpStrName )
	{
		if(pCalendar)
			pCalendar->SetName(lpStrName);
	}

	void CCalendarDlg::SetTrancateDate( LPCTSTR szBegin,LPCTSTR szEnd)
	{
		if(pCalendar)
			pCalendar->SetTrancateDate(szBegin,szEnd);
	}
	...
	CCalendarUI::CCalendarUI(void)
	{
		...
		m_DefaultStyle.szBeginDate = _T("");
		m_DefaultStyle.szEndDate = _T("");

		mSubTitleString.nSundayStr			= _T("{c #FFFFFF}日{/c}");
		mSubTitleString.nMondayStr			= _T("{c #FFFFFF}一{/c}");
		mSubTitleString.nTuesdayStr			= _T("{c #FFFFFF}二{/c}");
		mSubTitleString.nWednesdayStr		= _T("{c #FFFFFF}三{/c}");
		mSubTitleString.nThursdayStr		= _T("{c #FFFFFF}四{/c}");
		mSubTitleString.nFridayStr			= _T("{c #FFFFFF}五{/c}");
		mSubTitleString.nSaturdayStr		= _T("{c #FFFFFF}六{/c}");
		...
		pLastYearBtn->OnNotify	+= MakeDelegate(this,&CCalendarUI::OnLastYear,_T("click"));
		pMoothSelectBtn->OnNotify	+= MakeDelegate(this,&CCalendarUI::OnMoothSelect,_T("click"));
		pNextYearBtn->OnNotify	+= MakeDelegate(this,&CCalendarUI::OnNextYear,_T("click"));
		...
		for(int nMoothItem = 1;nMoothItem < 13;nMoothItem++)
		{
			...
			pMooth->OnNotify+= MakeDelegate(this,&CCalendarUI::OnSelectMooth,_T("click"));
	...
	{ 
		...
		pDay->OnNotify+= MakeDelegate(this,&CCalendarUI::OnSelcetDay,_T("click"));
	...
	void CCalendarUI::SetCalendarStyle( TCalendarStyle& _TCalendarStyle )
	{
		...
		pToDayBtn->SetAttribute(_T("hotbkcolor"),_TCalendarStyle.nDayHotColor);
		...
		pInfoPanelHorz->SetAttribute(_T("bkcolor"),_TCalendarStyle.nStatusBkColor);
		...
		time_t time_begin = 0;
		time_t time_end = 0;
		if(_TCalendarStyle.szBeginDate != _T(""))
		{
			swscanf_s(_TCalendarStyle.szBeginDate,_T("%d.%d.%d"),&tm_Begin.tm_year,&tm_Begin.tm_mon,&tm_Begin.tm_mday);
			tm_Begin.tm_year -= 1900;
			tm_Begin.tm_mon --;
			tm_Begin.tm_isdst=-1;
			time_begin = mktime(&tm_Begin);
		}
		if(_TCalendarStyle.szEndDate != _T(""))
		{
			swscanf_s(_TCalendarStyle.szEndDate,_T("%d.%d.%d"),&tm_End.tm_year,&tm_End.tm_mon,&tm_End.tm_mday);
			tm_End.tm_year -= 1900;
			tm_End.tm_mon --;
			tm_End.tm_isdst=-1;
		...
		for(int nWeekIndex = 0;nWeekIndex < 6;nWeekIndex++)
		{
			...
			for(int nDayIndex = 0;nDayIndex < 7;nDayIndex++)
			{
				...
				if(nDayIndex == 0 || nDayIndex == 6)
					TmpDef.SmallFormat(_T("{c #FF0000}%d{/c}"),pTCalendarInfo->nDay); 
				else
					TmpDef.SmallFormat(_T("%d"),pTCalendarInfo->nDay); 


				if(pTCalendarInfo->nAsMooth != pTCalendarInfo->nMooth)
				{
					pDay->SetAttribute(_T("bkcolor"),_TCalendarStyle.nNoCurMonthDayColor);
					pDay->SetMouseEnabled(GetEnabledMoothSel());
					TmpDef.SmallFormat(_T("{c #7b7b7b}%d{/c}"),pTCalendarInfo->nDay); 
				}
				else 
				{
					pDay->SetAttribute(_T("bkcolor"),_T(""));
					pDay->SetMouseEnabled(true);
				}

				pDay->SetText(TmpDef.GetData());
				pDay->SetAttribute(_T("hottextcolor"),_T("0xffffffff"));
				pDay->SetAttribute(_T("selectedtextcolor"),_T("0xffffffff"));
				...
				struct tm tm_Day = {0};
				time_t time_day = 0;
				tm_Day.tm_year = pTCalendarInfo->nYear;
				tm_Day.tm_mon = pTCalendarInfo->nAsMooth;
				tm_Day.tm_mday = pTCalendarInfo->nDay;
				tm_Day.tm_year -= 1900;
				tm_Day.tm_mon --;
				tm_Day.tm_isdst=-1;
				time_day = mktime(&tm_Day);

				if(time_begin !=0 && time_end != 0)
				{
					if(time_day < time_begin || time_day > time_end)
	...
	void CCalendarUI::RemoveAllSubTitleString()
	{
		...
		TCalendarInfo *pCalendarInfo = NULL;
		for(int i = 0 ; i < mTCalendarInfoArray.GetSize() ;i++)
		{
			pCalendarInfo = static_cast<TCalendarInfo *>(mTCalendarInfoArray.GetAt(i));
			if(pCalendarInfo)
			{
				delete pCalendarInfo;
				pCalendarInfo = NULL;
			}
		}
	...
	void CCalendarUI::InitCalendarDis( int _Year,int _Month )
	{
		...
		time_t time_begin = 0;
		time_t time_end = 0;
		if(m_DefaultStyle.szBeginDate != _T(""))
		{
			swscanf_s(m_DefaultStyle.szBeginDate,_T("%d.%d.%d"),&tm_Begin.tm_year,&tm_Begin.tm_mon,&tm_Begin.tm_mday);
			tm_Begin.tm_year -= 1900;
			tm_Begin.tm_mon --;
			tm_Begin.tm_isdst=-1;
			time_begin = mktime(&tm_Begin);
		}
		if(m_DefaultStyle.szEndDate != _T(""))
		{
			swscanf_s(m_DefaultStyle.szEndDate,_T("%d.%d.%d"),&tm_End.tm_year,&tm_End.tm_mon,&tm_End.tm_mday);
			tm_End.tm_year -= 1900;
			tm_End.tm_mon --;
			tm_End.tm_isdst=-1;
			time_end = mktime(&tm_End);
	...
	void CCalendarUI::InitDayPanel( CControlUI* _Control,bool bWeekEnd,int _Year,int _Mooth,int _Day,int _Week,int _WeekLine,int _AsMooth )
	{
		...
		if(_AsMooth != _Mooth)
			TmpDef.SmallFormat(_T("{c #7b7b7b}%d{/c}"),_Day); 
	...
	CControlUI* CCalendarUI::CreateInfoPanel()
	{
		...
		pToDayBtn->OnNotify			   += MakeDelegate(this,&CCalendarUI::OnToday,_T("click"));
	...
	void CCalendarUI::AnyCalendarStyle( LPCTSTR _StyleStr,TCalendarStyle* _TCalendarStyle /*= NULL*/ )
	{
		...
		pCalendarInfo->szBeginDate = m_DefaultStyle.szBeginDate;
		pCalendarInfo->szEndDate = m_DefaultStyle.szEndDate;
		...
		while( *_StyleStr != _T('\0') ) {
			...
			if( !sValue.IsEmpty() ) {
				...
				else if( sItem == _T("begindate")) pCalendarInfo->szBeginDate = sValue;
				else if( sItem == _T("enddate")) pCalendarInfo->szEndDate = sValue;
			}
			if( *_StyleStr++ != _T(' ') ) break;
		}
	...
	void CCalendarUI::ShowCalendarDlg( CButtonUI* _pControl,LPCTSTR lpStrName)
	{
		CCalendarDlg *pCalendarDlg=new CCalendarDlg(_pControl);
		pCalendarDlg->SetCalendarName((LPTSTR)lpStrName);
	}

	void CCalendarUI::SetTrancateDate( LPCTSTR szBegin,LPCTSTR szEnd)
	{
		m_DefaultStyle.szBeginDate = szBegin;
		m_DefaultStyle.szEndDate = szEnd;
	}
