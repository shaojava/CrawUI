...
namespace UiLib
{
	...
	{
		if( IsEnabled() && m_nLinks > 0 ) return IsWantCursor();
		else return 0;
	}
	...
	void CTextUI::PaintText()
	{
		...
		if( IsEnabled() ) {
			if( m_bShowHtml )
				m_pManager->GetRenderCore()->DrawHtmlText( m_pManager, rc, m_sText, m_dwTextColor, \
				m_rcLinks, m_sLinks, m_nLinks, m_uTextStyle,m_bTextGlow);
			else
				m_pManager->GetRenderCore()->DrawText(m_pManager, rc, m_sText, m_dwTextColor, \
				m_iFont, m_uTextStyle,m_bTextGlow);
		}
		else {
			if( m_bShowHtml)
				m_pManager->GetRenderCore()->DrawHtmlText( m_pManager, rc, m_sText, m_dwDisabledTextColor, \
				m_rcLinks, m_sLinks, m_nLinks, m_uTextStyle,m_bTextGlow);
			else
				m_pManager->GetRenderCore()->DrawText(m_pManager, rc, m_sText, m_dwDisabledTextColor, \
				m_iFont, m_uTextStyle,m_bTextGlow);
		}
	}
}
