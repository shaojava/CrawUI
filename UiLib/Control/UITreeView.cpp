...
namespace UiLib
{
	...
	bool CTreeNodeUI::AddAt( CControlUI* pControl, int iIndex )
	{
		...
		pControl->SetVisible(m_bIsVisable);
	...
	bool CTreeNodeUI::AddChildNode( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		if (_tcsicmp(_pTreeNodeUI->GetClass(), _T("TreeNodeUI")) != 0)
			return false;
		_pTreeNodeUI->SetVisible(m_bIsVisable);
		...
		if(pTreeView){
			...
			{
				if(GetTreeIndex() == pTreeView->GetCount()-1)
					nRet = pTreeView->Add(_pTreeNodeUI) >= 0;
				else
	...
	void CTreeNodeUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		...
		if( _tcscmp(pstrName, _T("textpadding")) == 0 ) {
			...
			if(pItemButton) pItemButton->SetTextPadding(rcTextPadding);
		}
	...
	CTreeNodeUI* CTreeNodeUI::CalLocation( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		int nCount = 0;
		CTreeNodeUI *pParentNodeCnter = _pTreeNodeUI->GetParentNode();
	...
	void CTreeViewUI::RemoveAll(bool bUpdate)
	{
		CListUI::RemoveAll(bUpdate);
	}
	...
	void CTreeViewUI::SetItemExpand( bool _Expanded,CTreeNodeUI* _TreeNode /*= NULL*/ )
	{
		if(_TreeNode)
		{
			if(_TreeNode->GetCountChild() > 0)
			{
				...
				_TreeNode->GetFolderButton()->SetCheck(_Expanded);
			}
		}
		else
		{
