...
namespace UiLib {
	...
	CRichEditWnd::CRichEditWnd(HWND hWndParent)
	{
		CreateDuiWindow(hWndParent,_T(""), WS_POPUP, WS_EX_LAYERED | WS_EX_NOACTIVATE);
		m_pRichEdit = NULL;
	}

	CRichEditWnd::~CRichEditWnd(void)
	{

	}

	CDuiString CRichEditWnd::GetSkinFile()
	{
		return CDuiString(_T("..//common//RichEditWnd.xml"));
	}

	CDuiString CRichEditWnd::GetSkinFolder()
	{
		return CDuiString(CPaintManagerUI::GetResourcePath());
	}

	LPCTSTR CRichEditWnd::GetWindowClassName() const
	{
		return _T("RichEditFlowWnd");
	}
	...
	void CRichEditWnd::Init()
	{
		::SetLayeredWindowAttributes(*this, RGB(255, 0, 255), 255, LWA_COLORKEY | LWA_ALPHA);
	...
	void CRichEditWnd::SetAlpha( int nOpticy )
	{
		pm.SetTransparent(nOpticy);
	}

	void CRichEditWnd::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		if(!m_pRichEdit)
			m_pRichEdit = static_cast<CRichEditUI *>(pm.FindSubControlByClass(NULL,_T("RichEditUI")));
		if(m_pRichEdit)
			m_pRichEdit->SetAttribute(pstrName,pstrValue);
	}
...
#include <textserv.h>
	...
	HRESULT InitDefaultCharFormat(CRichEditUI* re, CHARFORMAT2W* pcf, HFONT hfont) 
	{
		...
		LOGFONT lf;
		if( !hfont )
			hfont = re->GetManager()->GetFont(re->GetFont())->hFont;
...
err:
	...
	{
		if(m_re->m_pCaret)
			return m_re->m_pCaret->Create(hbmp,xWidth,yHeight);
		else
	...
	BOOL CTxtWinHost::TxShowCaret(BOOL fShow)
	{
		...
		{
			if(fShow)
				m_re->m_pCaret->Show(!m_re->m_bDuringTxDraw);
			else
				m_re->m_pCaret->Hide(!m_re->m_bDuringTxDraw);
			return TRUE;
		}
		else
		{
	...
	BOOL CTxtWinHost::TxSetCaretPos(INT x, INT y)
	{
		if(m_re->m_pCaret)
		{
			m_re->m_pCaret->SetPos(x,y,!m_re->m_bDuringTxDraw);
			return TRUE;
		}
		else
...
#ifdef _UNICODE
		_tcsncpy(cf.szFaceName, lf.lfFaceName,LF_FACESIZE);
#else
		//need to thunk pcf->szFaceName to a standard char string.in this case it's easy because our thunk is also our copy
	...
	void CRichEditUI::SetFont(int index)
	{
		m_iFont = index;
		if( m_pTwh ) 
			m_pTwh->SetFont(GetManager()->GetFont(m_iFont)->hFont);
	}
	...
	void CRichEditUI::DoInit()
	{
		if(m_bInited)
			return ;
		if(m_pManager->GetRenderCore()->IsWindowLayered())
			m_pCaret = new CCaretUI(m_pManager);
		CREATESTRUCT cs;
		...
		if( m_pTwh ) {
			if( msg == WM_KEYDOWN && TCHAR(wparam) == VK_RETURN ) {
				if( !m_bWantReturn || (::GetKeyState(VK_CONTROL) < 0 && !m_bWantCtrlReturn) )
				{
					if( m_pManager != NULL ) 
					{
						m_pManager->HandleIDOKEvent();
						m_pManager->SendNotify((CControlUI*)this, _T("editreturn"));
					}
					return S_OK;
				}
			}
		...
		case EN_DRAGDROPDONE:   
			{
				...
				{   
					LONG nId =  GetWindowLongPtr(this->GetManager()->GetPaintWindow(), GWL_ID);   
					NMHDR  *phdr = (NMHDR *)pv;   
			...
			break;
		case EN_CHANGE:
			SendMessage(this->GetManager()->GetPaintWindow(), WM_COMMAND, (WPARAM) iNotify, (LPARAM) this);
			break;
		}
	}
	...
	void CRichEditUI::DoPaint(const RECT& rcPaint)
	{
		RECT rcTemp = { 0 };
		if( !::IntersectRect(&rcTemp, &rcPaint, &m_rcItem) ) return;
		CRenderClip clip;
		clip.SetPaintManager(m_pManager);
		CRenderClip::GenerateClip(rcTemp, clip);
		...
		if( m_pTwh ) {
			...
			/* ITextServices2::TxDrawD2D Only For Win8,
			So,We Can Only Use DXGI Under D2D Render;
			*/
			...
			if(m_pManager->GetRenderCore()->IsWindowLayered())
			{
				...
				{
					BITMAP bmp;
					int nRes = ::GetObject(hBitmap,sizeof(BITMAP),&bmp);
					void *pBits = bmp.bmBits;
					if(nRes && pBits)
						CGenerAlgorithm::FixGDIAlpha(&pBits,(RECT*)&rcTemp,bmp.bmWidthBytes);
				}
			}
		...
		if( m_items.GetSize() > 0 ) {
			...
			if( !::IntersectRect(&rcTemp, &rcPaint, &rc) ) {
				...
				CRenderClip::GenerateClip(rcTemp, childClip);
				for( int it = 0; it < m_items.GetSize(); it++ ) {
					CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
					if( !pControl->IsVisible() ) continue;
					if( !::IntersectRect(&rcTemp, &rcPaint, &pControl->GetPos()) ) continue;
	...
	LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
	{
		...
		{
			//Fixed the input windows wrong Pos bug;
		...
		if( (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) 
		{
			...
			if( uMsg == WM_SETCURSOR || WM_MOUSEMOVE) bWasHandled = false;
