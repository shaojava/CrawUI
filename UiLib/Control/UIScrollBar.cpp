...
namespace UiLib
{
	...
	CScrollBarUI::CScrollBarUI() : m_bHorizontal(false), m_nRange(0), m_nScrollPos(0), m_nLineSize(8), 
		...
		m_uButton2State(0), m_uThumbState(0), m_bShowButton1(true), m_bShowButton2(true),m_bTakePlace(false)
	{
		m_nFlexRange = 120;
		m_nFlexedRange = 0;
	...
	void CScrollBarUI::SetHorizontal(bool bHorizontal)
	{
		...
		if( m_bHorizontal ) {
			if( m_cxyFixed.cy == -1 ) {
				m_cxyFixed.cx = 0;  //if Horizontal set height Default value;
				m_cxyFixed.cy = DEFAULT_SCROLLBAR_SIZE;
			}
		}
		else {
			if( m_cxyFixed.cx == -1 ) {  //else set Width Defalt value;
				m_cxyFixed.cx = DEFAULT_SCROLLBAR_SIZE;
				m_cxyFixed.cy = 0;
			}
		}
	...
	bool CScrollBarUI::SetScrollRange(int nRange)
	{
		bool bReCalPos = false;
		if( m_nRange == nRange ) return bReCalPos;
		m_nRange = nRange;
		if( m_nRange < 0 ) m_nRange = 0;
		if( m_nScrollPos > nRange) 
		{
			m_nScrollPos = nRange;
			bReCalPos = true;
		}
		SetPos(m_rcItem);
		if(nRange)
			m_pManager->SendNotify(m_pOwner->GetParent(),_T("ScrollBarInit"));
		return bReCalPos;
	}

	void CScrollBarUI::SetScrollRangeFlex(int nRange)
	{
	...
	void CScrollBarUI::SetScrollPos(int nPos,bool bFlex)
	{
		...
		if(bFlex)
		{
			if(m_nScrollPos < 0)
			{
				m_nFlexedRange = m_nScrollPos;
				if(m_nFlexedRange < -m_nFlexRange)
				{
					m_nScrollPos = -m_nFlexRange;
					m_nFlexedRange = -m_nFlexRange;
				}
			}
			if(m_nScrollPos > m_nRange)
			{
				m_nFlexedRange = m_nScrollPos-m_nRange;
				if(m_nFlexedRange > m_nFlexRange)
				{
					m_nFlexedRange = m_nFlexRange;
					m_nScrollPos = m_nRange+m_nFlexRange;
				}
			}
		}
		else
	...
	void CScrollBarUI::SetPos(RECT rc)
	{
		...
		if( m_bHorizontal ) 
		{
			...
			if( cx > m_cxyFixed.cy ) 
			{
				...
				if( m_nRange > 0 )
				{
					int cxThumb = (cx-abs(m_nFlexedRange)) * (rc.right - rc.left) / (m_nRange + rc.right - rc.left);
					if( cxThumb < m_cxyFixed.cy ) cxThumb = m_cxyFixed.cy;

					m_rcThumb.left = (m_nScrollPos-m_nFlexedRange) * (cx - cxThumb) / m_nRange + m_rcButton1.right;
			...
			if( cy > m_cxyFixed.cx ) 
			{
				...
				if( m_nRange > 0 ) 
				{
					int cyThumb = (cy-abs(m_nFlexedRange)) * (rc.bottom - rc.top) / (m_nRange + rc.bottom - rc.top);
					if( cyThumb < m_cxyFixed.cx ) cyThumb = m_cxyFixed.cx;

					m_rcThumb.top = (m_nScrollPos-m_nFlexedRange) * (cy - cyThumb) / m_nRange + m_rcButton1.bottom;
		...
		if( event.Type == UIEVENT_MOUSEMOVE )
		{
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
				if( !m_bHorizontal ) {
					int nButtonSize = 0;
					if(m_bShowButton1)
						nButtonSize += m_cxyFixed.cx;
					if(m_bShowButton2)
						nButtonSize += m_cxyFixed.cx;
					int vRange = m_rcItem.bottom - m_rcItem.top - m_rcThumb.bottom + m_rcThumb.top - nButtonSize;
					if (vRange != 0)
					...
					if(m_bShowButton2)
						nButtonSize += m_cxyFixed.cx;
					int hRange = m_rcItem.right - m_rcItem.left - m_rcThumb.right + m_rcThumb.left - nButtonSize;
					if (hRange != 0)
	...
	void CScrollBarUI::DoPaint(const RECT& rcPaint)
	{
	...
	void CScrollBarUI::PaintBk()
	{
		...
		if( (m_uThumbState & UISTATE_DISABLED) != 0 ) {
			if( !m_sBkDisabledImage.IsEmpty() ) {
				if( !DrawImage(m_sBkDisabledImage) ) m_sBkDisabledImage.Empty();
				else return;
			}
		}
		else if( (m_uThumbState & UISTATE_PUSHED) != 0 ) {
			if( !m_sBkPushedImage.IsEmpty() ) {
				if( !DrawImage(m_sBkPushedImage) ) m_sBkPushedImage.Empty();
				else return;
			}
		}
		else if( (m_uThumbState & UISTATE_HOT) != 0 ) {
			if( !m_sBkHotImage.IsEmpty() ) {
				if( !DrawImage(m_sBkHotImage) ) m_sBkHotImage.Empty();
				else return;
			}
		}

		if( !m_sBkNormalImage.IsEmpty() ) {
			if( !DrawImage(m_sBkNormalImage) ) m_sBkNormalImage.Empty();
			else return;
		}
	}

	void CScrollBarUI::PaintButton1()
	{
		...
		if( (m_uButton1State & UISTATE_DISABLED) != 0 ) {
			if( !m_sButton1DisabledImage.IsEmpty() ) {
				if( !DrawImage(m_sButton1DisabledImage, m_rcImageModify) ) m_sButton1DisabledImage.Empty();
				else return;
			}
		}
		else if( (m_uButton1State & UISTATE_PUSHED) != 0 ) {
			if( !m_sButton1PushedImage.IsEmpty() ) {
				if( !DrawImage(m_sButton1PushedImage, m_rcImageModify) ) m_sButton1PushedImage.Empty();
				else return;
			}
		}
		else if( (m_uButton1State & UISTATE_HOT) != 0 ) {
			if( !m_sButton1HotImage.IsEmpty() ) {
				if( !DrawImage(m_sButton1HotImage, m_rcImageModify) ) m_sButton1HotImage.Empty();
				else return;
			}
		}

		if( !m_sButton1NormalImage.IsEmpty() ) {
			if( !DrawImage(m_sButton1NormalImage, m_rcImageModify) ) m_sButton1NormalImage.Empty();
			else return;
		}

		DWORD dwBorderColor = 0xFF85E4FF;
		int nBorderSize = 2;
		m_pManager->GetRenderCore()->DrawRect(m_rcButton1, nBorderSize, dwBorderColor);
	}

	void CScrollBarUI::PaintButton2()
	{
		...
		if( (m_uButton2State & UISTATE_DISABLED) != 0 ) {
			if( !m_sButton2DisabledImage.IsEmpty() ) {
				if( !DrawImage(m_sButton2DisabledImage, m_rcImageModify) ) m_sButton2DisabledImage.Empty();
				else return;
			}
		}
		else if( (m_uButton2State & UISTATE_PUSHED) != 0 ) {
			if( !m_sButton2PushedImage.IsEmpty() ) {
				if( !DrawImage(m_sButton2PushedImage, m_rcImageModify) ) m_sButton2PushedImage.Empty();
				else return;
			}
		}
		else if( (m_uButton2State & UISTATE_HOT) != 0 ) {
			if( !m_sButton2HotImage.IsEmpty() ) {
				if( !DrawImage(m_sButton2HotImage, m_rcImageModify) ) m_sButton2HotImage.Empty();
				else return;
			}
		}

		if( !m_sButton2NormalImage.IsEmpty() ) {
			if( !DrawImage(m_sButton2NormalImage, m_rcImageModify) ) m_sButton2NormalImage.Empty();
			else return;
		}

		DWORD dwBorderColor = 0xFF85E4FF;
		int nBorderSize = 2;
		m_pManager->GetRenderCore()->DrawRect(m_rcButton2, nBorderSize, dwBorderColor);
	}

	void CScrollBarUI::PaintThumb()
	{
		...
		if( (m_uThumbState & UISTATE_DISABLED) != 0 ) {
			if( !m_sThumbDisabledImage.IsEmpty() ) {
				if( !DrawImage(m_sThumbDisabledImage, m_rcImageModify) ) m_sThumbDisabledImage.Empty();
				else return;
			}
		}
		else if( (m_uThumbState & UISTATE_PUSHED) != 0 ) {
			if( !m_sThumbPushedImage.IsEmpty() ) {
				if( !DrawImage(m_sThumbPushedImage, m_rcImageModify) ) m_sThumbPushedImage.Empty();
				else return;
			}
		}
		else if( (m_uThumbState & UISTATE_HOT) != 0 ) {
			if( !m_sThumbHotImage.IsEmpty() ) {
				if( !DrawImage(m_sThumbHotImage, m_rcImageModify) ) m_sThumbHotImage.Empty();
				else return;
			}
		}

		if( !m_sThumbNormalImage.IsEmpty() ) {
			if( !DrawImage(m_sThumbNormalImage, m_rcImageModify) ) m_sThumbNormalImage.Empty();
			else return;
		}

		DWORD dwBorderColor = 0xFF85E4FF;
		int nBorderSize = 2;
		m_pManager->GetRenderCore()->DrawRect(m_rcThumb, nBorderSize, dwBorderColor);
	}

	void CScrollBarUI::PaintRail()
	{
		if( m_rcThumb.left == 0 && m_rcThumb.top == 0 && m_rcThumb.right == 0 && m_rcThumb.bottom == 0 ) return;
		if( !IsEnabled() ) m_uThumbState |= UISTATE_DISABLED;
		else m_uThumbState &= ~ UISTATE_DISABLED;
		...
		if( (m_uThumbState & UISTATE_DISABLED) != 0 ) {
			if( !m_sRailDisabledImage.IsEmpty() ) {
				if( !DrawImage(m_sRailDisabledImage, m_rcImageModify) ) m_sRailDisabledImage.Empty();
				else return;
			}
		}
		else if( (m_uThumbState & UISTATE_PUSHED) != 0 ) {
			if( !m_sRailPushedImage.IsEmpty() ) {
				if( !DrawImage(m_sRailPushedImage, m_rcImageModify) ) m_sRailPushedImage.Empty();
				else return;
			}
		}
		else if( (m_uThumbState & UISTATE_HOT) != 0 ) {
			if( !m_sRailHotImage.IsEmpty() ) {
				if( !DrawImage(m_sRailHotImage, m_rcImageModify) ) m_sRailHotImage.Empty();
				else return;
			}
		}

		if( !m_sRailNormalImage.IsEmpty() ) {
			if( !DrawImage(m_sRailNormalImage, m_rcImageModify) ) m_sRailNormalImage.Empty();
			else return;
		}
	}

	void CScrollBarUI::ShowTakePlace( bool bTake )
	{
		m_bTakePlace = bTake;
	}

	bool CScrollBarUI::IsShowTakePlace()
	{
		return m_bTakePlace;
	}

	bool CScrollBarUI::IsTakePlace()
	{
		return m_bVisible&&m_bTakePlace&&(m_nRange != 0);
	}
